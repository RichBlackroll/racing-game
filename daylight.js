...
const palette = {
  ...
  duskHorizon: new THREE.Color(0xc99791), duskZenith: new THREE.Color(0x414569),
...
export function sampleDaylight(hour, level = "forest", result = {}) {
  ...
  result.period = elevation < -0.16 ? "night" : elevation < 0.07 ? (hour < 12 ? "dawn" : "blue-hour")
  ...
  result.horizon.copy(palette.nightHorizon).lerp(palette.dayHorizon, result.daylight)
    .lerp(palette.duskHorizon, result.twilight * 0.65);
  result.zenith.copy(palette.nightZenith).lerp(palette.dayZenith, result.daylight)
    .lerp(palette.duskZenith, result.twilight * 0.6);
...
/** Owns the game clock and lighting. Advance only with active simulation dt. */
  ...
  return {
    ...
    advance(dt) {
      if (!disposed && value.running && Number.isFinite(dt) && dt > 0)
        sampleDaylight(value.hour + Math.min(dt, 0.1) * 24 / (value.cycleMinutes * 60), level, value);
    },
    update,
    dispose() {
      if (disposed) return;
