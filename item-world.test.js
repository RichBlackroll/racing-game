...
test("exact API works in Node without a DOM, lights, cameras or an animation loop", t => {
  ...
  assert.equal(count(scene, "pickups/gift-boxes"), 60);
  assertFinite(scene);
});
...
for (const item of ITEMS) test(`${item.id}: real core deployment renders its distinct toy/status/effect and resets`, t => {
  ...
  assert.ok(count(scene, `pickups/hint/${item.id}`) > 0);
  ...
  if (item.id === "shield" || item.id === "star") {
    ...
    assert.equal(mesh(scene, `entities/${item.id}`).geometry, mesh(scene, `pickups/hint/${item.id}`).geometry);
  }
  for (let i = 0; i < 12; i++) {
...
test("available on/off-road gifts are compacted, instanced and correctly reconciled at dt=0", t => {
  const { scene, world, system } = fixture(t);
  const snapshot = system.state();
  world.update(snapshot, 0);
  assert.ok(snapshot.pickups.some(p => course.roadDistance(p.x, p.z) > 35));
  assert.ok(scene.getObjectByName("items/world").children.length <= 13, "60 detailed gifts use at most 13 draws");
  const color = new THREE.Color();
  snapshot.pickups.forEach((p, i) => {
    const position = positionAt(mesh(scene, "pickups/gift-boxes"), i);
    close(position.x, p.x); close(position.z, p.z); assert.ok(position.y > p.y);
    mesh(scene, "pickups/gift-boxes").getColorAt(i, color);
  ...
  world.update(snapshot, 0); assert.equal(count(scene, "pickups/gift-boxes"), 30);
  snapshot.pickups = snapshot.pickups.slice(0, 2).map(p => ({ ...p, available: true, x: p.x + 1000, y: 20 }));
  world.update(snapshot, 0); assert.equal(count(scene, "pickups/gift-boxes"), 2);
  close(positionAt(mesh(scene, "pickups/gift-boxes")).x, snapshot.pickups[0].x);
...
test("dt=0 and invalid dt freeze decoration; snapshots still move entities and racer overlays", t => {
  ...
  const giftBefore = matrixAt(mesh(scene, "pickups/gift-boxes")).elements;
  world.update(snapshot, 0.1, racers);
  assert.notDeepEqual(matrixAt(mesh(scene, "pickups/gift-boxes")).elements, giftBefore);
  world.update(snapshot, 0, []);
...
test("hundreds of pickups and capped effects reuse bounded geometry/material/instance resources", t => {
  ...
  assert.equal(geometries.size, 14); assert.equal(materials.size, 3);
  assert.ok(meshes.length <= 25); assert.equal(count(scene, "effects/confetti"), 384);
...
test("growing instance buffers disposes old meshes without disposing shared geometry", t => {
  ...
  const old = mesh(scene, "pickups/gift-boxes"); let meshDisposals = 0, geometryDisposals = 0;
  ...
  assert.equal(mesh(scene, "pickups/gift-boxes").geometry, old.geometry);
  assert.equal(count(scene, "pickups/gift-boxes"), 60); assertFinite(scene);
...
test("invalid coordinates are skipped and missing optional headings never create NaN transforms", t => {
  ...
  assert.equal(count(scene, "pickups/gift-boxes"), 0); assert.equal(count(scene, "entities/ball"), 0);
