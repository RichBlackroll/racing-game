...
function fixture() {
  const elements = Object.fromEntries(["item-slot", "use-item", "item-icon", "item-name", "item-action", "item-hint", "item-status"]
...
test("protective item countdowns are readable and clear when their timers expire", () => {
  ...
  ui.update(state, modifiers);
  assert.equal(e["item-status"].textContent, "10 surprises to discover");
});
...
test("production lifecycle never advances items in a pause, loading screen, garage, or hidden tab", async () => {
  ...
  for (const mode of ["paused", "loading", "hidden", "contextLost", "adventure", "garage"]) {
    const context = { requestAnimationFrame() {}, itemUI: { update() {} }, itemSnapshot: {},
      ...
      document: { hidden: mode === "hidden" }, contextLost: mode === "contextLost", adventure: { busy: () => mode === "adventure" },
  ...
  assert.match(source, /friendRacers\.reset\(car\.position\);\s*resetItems\(\)/);
});
